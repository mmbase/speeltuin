 ...
 if (target != "") {
 	if (cmd=="kill") {
 		if (confirm("Are you sure?")) {
     		top[target].location.href="submit.jsp?q=" + cmd;
 	    }
 	} else {
     top[target].location.href="submit.jsp?q=" + cmd;
 	}
 } else {
 location.href="submit.jsp?q=" + cmd;
 }               
}
var _tabLast=null;
function _rObj (s,ar) {
...
top.fstatus.document.writeln("<link href='css/style.css' rel='stylesheet' type='text/css'>");
top.fstatus.document.writeln("</head><body><center><table width=99% border=0 cellspacing=0 cellpadding=0>");
top.fstatus.document.writeln("<form action='submit.jsp' target=output name=cmdFormular> " );
top.fstatus.document.writeln("<tr><td width=100% nowrap>");
top.fstatus.document.writeln("<input type='hidden' name='q' value='beanshell'/>");
top.fstatus.document.writeln(" <input class='txt' style='width: 99%; height: 20px; font-size: 12px;'"); 
top.fstatus.document.writeln(" type=text name='exec' value=''> </td><td width=1>");
