 ...
 */
public class InstallApplication extends ApplicationTask {

    	private static Logger log = Logging.getLoggerInstance(InstallApplication.class.getName());

	public InstallApplication(int taskid,ApplicationTaskManager atm, MMBase mmb) {
	 ...
	 */
	public void doWork() {
		setState("ready","install application task, waiting for init vars");
		...
		try{
                	application.install();
			setState("done","application install done");
		} catch (Exception excep ) {
			setField("ERRORMSG",excep.getMessage());
			setState("error","application install exception");
		}
	}
