 ...
 */
public class InstallDataSet extends ApplicationTask {

    	private static Logger log = Logging.getLoggerInstance(InstallDataSet.class.getName());

	public InstallDataSet(int taskid,ApplicationTaskManager atm, MMBase mmb) {
	 ...
	 */
	public void doWork() {
		setState("ready","install dataset task, waiting for init vars");
		...
		try{
                	element.install();
			setState("done","dataset install done");
		} catch (Exception excep ) {
			setField("ERRORMSG",excep.getMessage());
			setState("error","dataset install exception");
		}
	}
