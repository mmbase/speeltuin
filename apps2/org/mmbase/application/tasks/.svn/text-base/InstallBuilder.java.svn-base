 ...
 */
public class InstallBuilder extends ApplicationTask {

    	private static Logger log = Logging.getLoggerInstance(InstallBuilder.class.getName());

	public InstallBuilder(int taskid,ApplicationTaskManager atm, MMBase mmb) {
	 ...
	 */
	public void doWork() {
		setState("ready","builder install task, waiting for init vars");
		...
		try{
                	element.installBuilder(i);
			setState("done","builder install done");
		} catch (Exception excep ) {
			setField("ERRORMSG",excep.getMessage());
			setState("error","builder install exception");
		}
	}
