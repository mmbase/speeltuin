 ...
 */
public class InstallDisplay extends ApplicationTask {

    	private static Logger log = Logging.getLoggerInstance(InstallDisplay.class.getName());

	public InstallDisplay(int taskid,ApplicationTaskManager atm, MMBase mmb) {
	 ...
	 */
	public void doWork() {
		setState("ready","install display task, waiting for init vars");
		...
		try{
                	element.install();
			setState("done","display install done");
		} catch (Exception excep ) {
			setField("ERRORMSG",excep.getMessage());
			setState("error","display install exception");
		}
	}
