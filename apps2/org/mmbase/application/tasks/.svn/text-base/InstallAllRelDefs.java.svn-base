 ...
 */
public class InstallAllRelDefs extends ApplicationTask {

    	private static Logger log = Logging.getLoggerInstance(InstallAllRelDefs.class.getName());

	public InstallAllRelDefs(int taskid,ApplicationTaskManager atm, MMBase mmb) {
	 ...
	 */
	public void doWork() {
		setState("ready","install all reldefs task, waiting for init vars");
		...
		try{
                	element.installRelDefs();
			setState("done","install reldefs done");
		} catch (Exception excep ) {
			setField("ERRORMSG",excep.getMessage());
			setState("error","install reldefs exception");
		}
	}
