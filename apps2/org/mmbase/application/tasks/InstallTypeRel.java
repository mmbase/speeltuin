 ...
 */
public class InstallTypeRel extends ApplicationTask {

    	private static Logger log = Logging.getLoggerInstance(InstallTypeRel.class.getName());

	public InstallTypeRel(int taskid,ApplicationTaskManager atm, MMBase mmb) {
	 ...
	 */
	public void doWork() {
		setState("ready","typerel install task, waiting for init vars");
		...
		try{
                	element.installTypeRelById(i);
			setState("done","typerel install done");
		} catch (Exception excep ) {
			setField("ERRORMSG",excep.getMessage());
			setState("error","typerel install exception");
		}
	}
