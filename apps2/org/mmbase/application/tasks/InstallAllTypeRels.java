 ...
 */
public class InstallAllTypeRels extends ApplicationTask {

    	private static Logger log = Logging.getLoggerInstance(InstallAllTypeRels.class.getName());

	public InstallAllTypeRels(int taskid,ApplicationTaskManager atm, MMBase mmb) {
	 ...
	 */
	public void doWork() {
		setState("ready","install typerels task, waiting for init vars");
		...
		try{
                	element.installTypeRels();
			setState("done","install all typerels done");
		} catch (Exception excep ) {
			setField("ERRORMSG",excep.getMessage());
			setState("error","install typerels exception");
		}
	}
