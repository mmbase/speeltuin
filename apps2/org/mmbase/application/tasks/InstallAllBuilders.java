 ...
 */
public class InstallAllBuilders extends ApplicationTask {

    	private static Logger log = Logging.getLoggerInstance(InstallAllBuilders.class.getName());

	public InstallAllBuilders(int taskid,ApplicationTaskManager atm, MMBase mmb) {
	 ...
	 */
	public void doWork() {
		setState("ready","install all builders task, waiting for init vars");
		...
		try{
                	element.installBuilders();
			setState("done","builders install done");
		} catch (Exception excep ) {
			setField("ERRORMSG",excep.getMessage());
			setState("error","builders install exception");
		}
	}
