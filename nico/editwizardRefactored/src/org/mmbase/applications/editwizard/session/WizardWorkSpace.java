...
/**
 * Performs actions on the data stored in the session, suing info gaiend from the parent wizard configuration (WizardConfig) object.
 * @javadoc
 * @author Finalist
 * @version $Id: WizardWorkSpace.java,v 1.1 2006-02-02 12:18:33 pierre Exp $
 */
public class WizardWorkSpace {
    ...
    public WizardWorkSpace(WizardConfig config) {
     ...
     */
    public boolean doLoad(Cloud cloud) throws WizardException {
        ...
        if ("new".equals(wizardConfig.objectNumber)) {
            ...
            wizardData = connector.load(objectElm,wizardConfig.objectNumber);
        }
    ...
    /**
     * Determines whether the wizard data has been loaded.
     * This should return <code>true</code> once {@link #doLoad} has been called.
     */
    ...
    /**
     * Returns the status of the wizard data, or {@link WizardData.STATUS_NOT_LOADED} if the data is not yet loaded.
     */
    public int getStatus() {
        if (!isLoaded()) {
            return BaseData.STATUS_NOT_LOADED;
        } else {
            return wizardData.getStatus();
        }
    }

    /**
     * Returns the number of the main object being edited;
     * @throws WizardException if the object data is not yet loaded.
     */
    public String getObjectNumber() throws WizardException {
        if (!isLoaded()) {
            throw new WizardException("Data has not yet been loaded.");
        } else {
            return wizardData.getNumber();
        }
    }
    ...
    public Document getDocument(Cloud cloud) throws WizardException {
        if (!isLoaded()) {
            ...
            WizardDOM wizarddom = WizardDOM.getInstance(wizardConfig, wizardData, cloud);
    ...
    /**
     * Store binary data for a specified field.
     */
    public void storeBinaryField(String did, BinaryData binaryData) {
        wizardData.findFieldById(did).setBinary(binaryData);
    }
     ...
     * @throws WizardException
     */
    public boolean saveWizardData(ServletRequest request, String timezone) throws WizardException {
    ...
    private void storeValue(String did, String fid, String value) throws WizardException {
        ...
        if (!(element instanceof FieldElm)) {
...
//        element.getAttribute(SchemaKeys.ATTR_DTTYPE);
        ...
        FieldData fieldData = wizardData.findFieldById(did);
...
//            }
    ...
    public boolean doSave(Cloud cloud) throws WizardException {

        WizardCloudConnector connector = WizardCloudConnector.getInstance(cloud);
        connector.save(wizardData);
     ...
     * @throws WizardException
     */
    public boolean doUpdateItem(Cloud cloud, String fid, String did, String number,
        ...
        if (!mainObject.getNumber().equals(origin)) {
    ...
    public boolean doMoveItem(String fid, String did, String otherid) throws WizardException{
        ...
        SchemaElement element = schema.findElementByFid(fid);
        BaseData baseData = DataUtils.findDataById(wizardData,did);
        BaseData otherData = DataUtils.findDataById(wizardData,otherid);

        if (!(baseData instanceof RelationData && otherData instanceof RelationData)) {
