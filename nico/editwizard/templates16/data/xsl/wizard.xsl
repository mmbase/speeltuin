...
<xsl:stylesheet 
  ...
  <!--
    ...
    @author Nico Klasens
    @version $Id: wizard.xsl,v 1.1 2003-12-02 14:39:07 nico Exp $
  -->
...
-->
        ...
        <tr>
          <td>
              <nobr>
                ...
                -
                <!-- Saveonly  -->
                <!-- 1.6 xsl:call-template name="saveonlybutton"/-->
              </nobr>
          </td>
        </tr>
      </table>
  ...
  <!-- 1.6 xsl:template name="saveonlybutton">
...
-->
  ...
  <xsl:template match="prefix|postfix">
	...
	<xsl:template name="ftype-image">
		<!-- 1.6 xsl:if test="@maywrite!=&apos;false&apos;" -->
			<xsl:choose>
	...
	<xsl:template name="ftype-file">
		<xsl:choose>
			<!-- 1.6 xsl:when test="@dttype=&apos;data&apos; or @maywrite=&apos;false&apos;" -->
  ...
  <xsl:template name="deleteitembuttons">
		<td>
			<xsl:if test="command[@name=&apos;delete-item&apos;]">
				<!-- 1.6 xsl:if test="@maydelete!=&apos;false&apos;" -->
					<span
						class="imgbutton"
  ...
  <xsl:template name="positembuttons">
		<td>
			<xsl:choose>
				<!-- 1.6 xsl:when test="@maywrite=&apos;true&apos; and command[@name=&apos;move-up&apos;]" -->
				<xsl:when test="command[@name=&apos;move-up&apos;]">
					<span
						class="imgbutton"
						title="{$tooltip_up}"
		...
		<td>
			<xsl:choose>
				<!-- 1.6 xsl:when test="@maywrite=&apos;true&apos; and command[@name=&apos;move-down&apos;]" -->
				<xsl:when test="command[@name=&apos;move-down&apos;]">
					<span
						class="imgbutton"
						title="{$tooltip_down}"
